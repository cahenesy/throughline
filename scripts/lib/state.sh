...
# block. The model default follows the build model (ADR 0008 — author↔reviewer
# diversity), falling back to the latest-tier literal where MODEL is unset
# (TDD 0057); it is distinct from the THROUGHLINE_REVIEW_MODEL alias — only
# the alias is snapshotted.
_rework_config_json() {
...
_write_run_fragment() {
  ...
  if [ -d "$STATE_DIR" ]; then
    for f in "$STATE_DIR"/*.json; do
      ...
      st="$(_read_fragment_field "$f" status)"
      case "$st" in
...
state_init() {
  ...
  if [ "${RESUME:-0}" -eq 1 ] && [ -f "$STATE_DIR/run.json" ]; then
    ...
    STATE_MODE="$(_read_fragment_field "$STATE_DIR/run.json" mode)"
    ...
    _resume_change="$(_read_fragment_field "$STATE_DIR/run.json" change)"
...
set_tdd_state() {
  ...
  log="$(_read_fragment_field "$f" log)"
