...
# Sourced library modules (TDD 0015 + 0016 / Theme D). state.sh holds the atomic
# run-state / per-TDD fragment I/O cluster; pause-retry.sh holds the
# recoverable-failure classification + paused/retry state-transition cluster and
# is sourced AFTER state.sh because every function in it calls a state.sh helper.
...
# leaving every state-tracking / pause-retry helper undefined and the whole run
# silently no-op. pwd -P normalizes symlinks so a wrapper-invoked SCRIPT_DIR still
# resolves to the real lib/ dir.
