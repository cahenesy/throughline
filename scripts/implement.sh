...
# THROUGHLINE_BUILD_MODEL / THROUGHLINE_REVIEW_MODEL. Resolves the MODEL /
# REVIEW_MODEL globals in place from the arg-parse state ("" = no flag passed).
# Defined above the SOURCE_ONLY guard so the test suite can drive it in
# isolation; the setup block calls it where the inline block used to sit.
resolve_models() {
