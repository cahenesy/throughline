...
# These run first so a red-first pass fails closed without executing the
# retired [A]–[S] coprocess scenarios (those spawn a stub `claude -p`).
echo "[0063] soft delegates and coprocess retirement"
ROOT63="$(mktemp -d)"
# Isolated fixture repo: never run the still-live implement.sh against this
# checkout (claude/grok are on PATH here and would start a real build).
...
# Recording stubs — must not be invoked by the retired runners.
...
# Match the live `bash "$BCL"` call, not this assertion's quoted pattern.
if grep -q 'bash "\$BCL"' "$0"; then
  bad "aggregator still invokes the coprocess-lifecycle eval"
else
  ok "aggregator does not invoke build-coprocess-lifecycle"
fi

# 5. 0006 Status superseded by 0063 (may already hold on master)
...
if grep -qiE 'optional' "$REPO/README.md" \
   && grep -qE 'superpowers|pr-review-toolkit' "$REPO/README.md"; then
  # Require optional + the plugin names in close proximity (same or adjacent lines).
  ...
  bad "README missing optional-delegate wording"
fi
...
# Fail closed before the retired coprocess scenarios / chained evals.
if grep -q '^fail$' "$RESULTS" 2>/dev/null; then
  PASS="$(grep -c '^ok$'   "$RESULTS" 2>/dev/null)"; PASS="${PASS:-0}"
  FAIL="$(grep -c '^fail$' "$RESULTS" 2>/dev/null)"; FAIL="${FAIL:-0}"
  echo
  echo "=== 0063 observations: $PASS passed, $FAIL failed (skipping coprocess scenarios) ==="
  rm -f "$RESULTS"
  exit 1
fi
