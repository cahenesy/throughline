...
EOF
  ...
  grep -q '^IMPLEMENT_RUN_COMPLETE' "$out" 2>/dev/null && bad "no completion line should be emitted while the build is progressing" || ok "no premature IMPLEMENT_RUN_COMPLETE while progressing"
